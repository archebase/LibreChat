...
jest.mock('~/hooks/Messages/useFeishuDocImport', () => ({
  __esModule: true,
  default: () => ({
...
describe('useSubmitMessage', () => {
  ...
  it('propagates blocked submits so direct callers can preserve their text', async () => {
    ...
    await act(async () => {
      submitted = await result.current.submitMessage({ text: 'dictated follow-up' });
    });

    expect(mockImportFeishuDocsForMessage).toHaveBeenCalledWith('dictated follow-up');
